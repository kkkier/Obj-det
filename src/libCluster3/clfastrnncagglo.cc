...
ClFastRNNCAgglo::ClFastRNNCAgglo( const vector<FeatureVector> &vPoints,
                                  int nMaxNodeSize )
  : Cluster( vPoints )
  , m_btNNTree()
{
...
void ClFastRNNCAgglo::initDataVectors()
{
...
void ClFastRNNCAgglo::initClusterCenters()
{
...
void ClFastRNNCAgglo::initCovariances()
{}


void ClFastRNNCAgglo::initPriors()
{}


void ClFastRNNCAgglo::initPosteriors()
{
  //doReestimationStep();
}


float ClFastRNNCAgglo::sim( int idx1, int idx2 )
{
  float sim = MIN_SIM;
...
void ClFastRNNCAgglo::agglomerate( int idx1, int idx2, int newidx )
  /*******************************************************************/
...
/*************************************************************/
bool ClFastRNNCAgglo::doClusterSteps( double minSimilarity, bool bVerbose )
{
  // TIMING CODE
  ...
  m_bMore = true;
  if( bVerbose )
    cout << "  ClFastRNNCAgglo::doClusterSteps() called with min similarity: " 
...
//       }
    int   nnidx2 = vNN[last];
    float nnsim2 = vSim[last]; 
    unsigned nResult;
    float    dDist;
    //m_btNNTree.clearAllVisited();
    if( m_btNNTree.findNN( vNN[last], -vSim[last], nResult, dDist ) ) {
      nnidx2 = (int)nResult;
      nnsim2 = -dDist; //sim( vNN[last], nnidx2 );
    }
    
//     float    dEps = 0.01;
//     if( (fabs(nnsim-nnsim2)>dEps) && (nnsim>vSim[last]) ) {
//       cout << "  Error searching for NN(" << vNN[last] << "," << -vSim[last]
//            << "): wrong result!" << endl
//            << "    nnidx= " << nnidx2 << " instead of " << nnidx << endl
//            << "    nnsim=" << nnsim2 << " instead of " << nnsim << endl;
//       cout << "    incorrectly returned node: " << endl;
//       m_btNNTree.printStorageNodeDist( nnidx2, vNN[last] );
//       m_btNNTree.printTraceToRoot( nnidx2 );
//       cout << "    correct node: " << endl;
//       m_btNNTree.printStorageNodeDist( nnidx, vNN[last] );
//       m_btNNTree.printTraceToRoot( nnidx );
//       exit(0);
//     }
    int   nnidx = nnidx2;
    float nnsim = nnsim2;
    ...
    if( last < 0 ){
      ...
      m_btNNTree.makeInactive( nFirstActive );
      nNumActive--;
...
void ClFastRNNCAgglo::doReestimationStep()
{}
...
void ClFastRNNCAgglo::doUpdateStep()
{
}
...
FLOAT ClFastRNNCAgglo::calculateError()
{
  return -1.0;
}
...
void ClFastRNNCAgglo::printResults()
{
