...
// ======================================================================*
//
// Doron Tal added wrappers and converted the to C++.  This fft
// routine does an N-Dimensional fft with arbitrary number of points
// at each dimension (not just powers of 2).
...
// some stuff has been commented out here (-dt) see the original
// files for some aweful hack, if you wish to compile this for 
// double-precision
...
// NB: if you define FFTRADIX to be fftradixf, comment out the 
// definition of fftradix above, and vice versa, to avoid 'defined
// but not declared' compiler warnings --DT
 ...
 *
 */
int DTLib::FFTN (int ndim, const int dims[],
                 REAL Re [],
                 REAL Im [],
                 int iSign,
                 double scaling)
{
    ...
    for (i = 0; i < ndim; i++)
        {
            nSpan = dims [i];
            if ((int)nSpan > max_factors) max_factors = nSpan;
            if ((int)nSpan > max_perm) max_perm = nSpan;
        }
#else
          ...
          int max_perm)
{
    ...
    REAL s60 = (REAL)SIN60;    /* sin(60 deg) */
    REAL c72 = (REAL)COS72;    /* cos(72 deg) */
    REAL s72 = (REAL)SIN72;    /* sin(72 deg) */
    REAL pi2 = (REAL)M_PI;   /* use PI first, 2 PI later */
    ...
    if (mfactor > NFACTOR)
        {
            fputs ("Error: "
                   FFTRADIXS 
                   "() - exceeded number of factors\n", stderr);
            goto Memory_Error_Label;
        }

    /* compute fourier transform */
    for (;;) {
        sd = (REAL)(radf / (double) kspan);
        cd = (REAL)sin(sd);
        cd = (REAL)2.0 * cd * cd;
        sd = (REAL)sin(sd + sd);
        kk = 1;
        ...
        case 2:
            ...
            if (kk > kspan)
                goto Permute_Results_Label;    /* quit infinite loop */
            do {
                c1 = (REAL)1.0 - cd;
                s1 = sd;
                do {
