...
{  
  ...
  connect( this, SIGNAL(mouseClickOnImage(int)), 
...
void QtClusterView::loadImageSets( vector<OpGrayImage> vImages, 
                                   string sImgName,
                                   vector<OpGrayImage> vAssociatedImgs,
                                   vector<vector<int> > vAssignment,
								   vector<string> vClusterInfo )
/*====================================================================*/
/* Load the cluster informations and associated patches. 
 * vImages are the Icon image of each cluster, 
 * sImgName are the name of the cluster, 
 * vAssociatedImgs are the original patch set,
 * vAssignment is the cluster assignment of each original patch
 * vClusterInfo contains the infomation of each leaf node including 
 * tree number and level et al.										  */
/*====================================================================*/
{
  if( vImages.empty() || (vAssignment.size() != vAssignment.size()) ) {
    cerr << "  Error in QtClusterView::loadImageSets(): " 
         << "vector dimensions must agree!" << endl;
    return;
  }
  ...
  for(int i=0; i < vImages.size(); i++) {
    ...
    QString sImgstr = QString::number( vAssignment[i].size() );
    string imgstr( sImgstr.latin1() );
    imgstr.insert(0, "\n");
    imgstr.insert(0, sKey.latin1());
    imgstr.insert(0, "_");
    imgstr.insert(0, sImgName);
	imgstr.insert(0, "\n");
	imgstr.insert(0, vClusterInfo[i]);
...
void QtClusterView::displayAssociatedImgs( int nImgId )
  ...
  /* just clicked.                                                   */
{
  if( !m_vvAssignment.empty()){
	vector<OpGrayImage> vCorresponding;

		for ( int i=0; i< m_vvAssignment[nImgId].size();i++)
		vCorresponding.push_back( m_vAssociatedImgs[m_vvAssignment[nImgId][i]] ); 
    displayImgsForBrowsing( vCorresponding );
	}
	else if( !m_vAssociatedImgs.empty() && !m_vAssignment.empty() ) {
		vector<OpGrayImage> vCorresponding;
		for ( int i=0; i < m_vAssignment.size(); i++ )
		if ( m_vAssignment[i] == nImgId )
			vCorresponding.push_back( m_vAssociatedImgs[i] );
