...
{  
  ...
  connect( this, SIGNAL(itemClicked(QListWidgetItem*)), 
           this, SLOT(mouseClickOnItem(QListWidgetItem*)));  
}


void QtIconView::mouseClickOnItem( QListWidgetItem *item)
{
  if( item != 0 ) {
    int imgNo = item->toolTip().toInt();
    emit( mouseClickOnImage( imgNo ));
  }
}
  ...
  /* load an image, convert it to a thumbnail, and add it to the box */
{
  ...
  QListWidgetItem *newItem = new QListWidgetItem( QIcon(pmThumb), sText.c_str(), this );
  newItem->setToolTip( sKey.c_str() );
}
  ...
  /* load an image, convert it to a thumbnail, and add it to the box */
{
  ...
  QListWidgetItem *newItem = new QListWidgetItem( QIcon(pmThumb), "", this );
  newItem->setToolTip( sKey.c_str() );
}
